...
const Navbar = () => {
  ...
  return (
    <header className="font-poppins w-full flex items-center justify-between bg-white px-[5%] py-[2%] shadow-lg">
      ...
      <ul className="flex items-center gap-3">
        {iconLinks.map((iconLink, index) => (
          <Link key={index} href={iconLink.url}>
            <li className={`mx-2 text-[#000000] hover:text-[#B88E2F] transition-all duration-300 ease-in-out ${index !== 3 ? "hidden md:block" : "block"}`}>
              {iconLink.icon}
            </li>
          </Link>
        ))}
      ...
      <button onClick={connectWallet} className="px-4 py-2 bg-[#B88E2F] text-white rounded-lg">
        {wallet ? "Wallet Connected" : "Connect Argent"}
      </button>
